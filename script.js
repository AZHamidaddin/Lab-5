...
inputSearchElem.addEventListener("keydown", applyFilters);

const darkModeToggle = document.getElementById("dark-mode-toggle");
darkModeToggle.addEventListener("click", toggleDarkMode);

const playerTableBody = document.querySelector("#player-rows");
...
selectTeamElem.addEventListener("change", applyFilters);
...
function populateTeamDropdown() {
    ...
    teams.forEach((team) => {
        ...
        selectTeamElem.appendChild(option);
    });
}

function applyFilters() {
    let searchTerm = inputSearchElem.value.toLowerCase();
    let selectedTeam = selectTeamElem.value;

    let filteredPlayers = data.filter((player) => {
        let matchesSearch = player.name.toLowerCase().includes(searchTerm);
        let matchesTeam =
            selectedTeam === "all" ||
